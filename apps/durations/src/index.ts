...
import { DurationsLogic } from './durations-logic';
...
app.get('/redirect', async (c) => {
  try {
    ...
    // Store user in database with 'labels' mode (since we removed project support)
    await db.addUser(user.id, tokenData.access_token, user.full_name, 'labels');
    
    // Initialize duration labels for the user
    const durationsLogic = new DurationsLogic(tokenData.access_token);
    await durationsLogic.ensureDurationLabels();
...
app.post('/webhook', async (c) => {
  try {
    ...
    const durationsLogic = new DurationsLogic(user.access_token);
    
    // Run in the background (Workers will handle this)
    c.executionCtx.waitUntil(
      durationsLogic.setTaskDuration(taskId).catch(error => 
