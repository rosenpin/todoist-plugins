...
app.post('/webhook', async (c) => {
  try {
    ...
    if (!userId || !taskId || checked === undefined) {
    ...
    const doneLogic = new DoneLogic(user.access_token);
    
    // Run in the background (Workers will handle this)
    c.executionCtx.waitUntil(
      doneLogic.processTask(taskId, checked).catch(error => 
        Logger.error('Error processing webhook task:', error, { userId, taskId, checked })
      )
    );
